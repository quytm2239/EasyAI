package easy.connectivity.websocket;
...
import easy.connectivity.inteface.PingSenderInterface;
import easy.connectivity.inteface.WSConnectionInterface;
import easy.connectivity.inteface.WSSessionEventInterface;
import easy.connectivity.ping.PingSender;
...
public class WSConnection implements WSSessionEventInterface, PingSenderInterface {
	...
	public void onPing() {
		...
		// This below line will make new connect if not ManualDisconnect and current
		// status is DISCONNECTED
		if (!this.isManualDisconnect() && this.getStatus() == WSConnectionStatus.DISCONNECTED && this.isReady())
			this.connect();
	}
	...
	public void onSocketConnect(Session session) {
		...
		if (this.getWsConnectionInterface() != null)
			this.getWsConnectionInterface().onConnect();
	...
	public void onSocketClose(int statusCode, String reason) {
		...
		if (!this.isManualDisconnect() && this.getStatus() == WSConnectionStatus.CONNECTED)
			this.connect();
		this.setStatus(WSConnectionStatus.DISCONNECTED);
		if (this.getWsConnectionInterface() != null)
			this.getWsConnectionInterface().onDisconnected();
	}
	...
	public void onSocketMessage(String data) {
		...
		if (this.getWsConnectionInterface() != null)
			this.getWsConnectionInterface().onIncomingData(data);
	}
